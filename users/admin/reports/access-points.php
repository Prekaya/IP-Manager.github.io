	...
	<body>
		<header>
			<nav>
				<ul>
					...
					<li class = 'selected'>Access Points</li>
		...
		<form class = 'form' action='' method='post'>
			...
			<div class='form-row' >
				...
				<select id='order' name='order' onchange="this.form.submit();">
					<option <?php if ($sort == 'ASC'){echo 'selected';}?> value="ASC">Ascending</option>
					<option <?php if ($sort == 'DESC'){echo 'selected';}?> value="DESC">Descending</option>
				</select>
			</div>
		</form>
		...
		<table>
			...
			<?php
				...
				$stmt = $db->query("SELECT * FROM devices JOIN ips ON devices.dIPID = ips.ipID WHERE dGID = 3 ORDER BY $sort $order");
